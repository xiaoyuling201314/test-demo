...
function initMap() {
    ...
    function(c) {
        ...
        for (var i = 0; i < allOverlay.length; i++){
            if(allOverlay[i].getTitle() == "clickPoint"){
                map.removeOverlay(allOverlay[i]);
            }
        }
        //创建点击坐标
        var clickMarker = new BMap.Marker(new BMap.Point(b.lng, b.lat));
        clickMarker.setTitle("clickPoint");
        map.addOverlay(clickMarker);

        Fe.G("selPointTitle").value = "";
        Fe.G("selPointAddress").value = "";
    });
...
function setInputPoint(a, address) {
    //设置中心点
    map.panTo(new BMap.Point(a.lng, a.lat));
...
function bindEvent() {
    ...
    var b = function(g) {
        ...
        if (a[g].K && a[g].K.title) {
            var title = a[g].K.title;
            title = title.substring(title.indexOf(">")+1, title.length);
            title = title.substring(0, title.lastIndexOf("<")).replace("<","");
            Fe.G("selPointTitle").value = title;
        }
        if (a[g].content) {
            var address = a[g].content;
            address = address.substring(0, address.indexOf("<br/>"));
            address = address.substring(address.lastIndexOf("</em>") + 5, address.length);
            Fe.G("selPointAddress").value = address;
        }
...
function addMarker(a) {
    var b = temp.pt[a];
    var d = new BMap.Icon("../../plug-in/Images/markers.png", new BMap.Size(23, 25), {
...
function goCity(c) {
    ...
    if(window.localStorage){
        window.localStorage["baiduMapDefaultCity"] = a;
    }
}
