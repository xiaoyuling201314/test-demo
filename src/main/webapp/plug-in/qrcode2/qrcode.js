...
for(function(a){
	a.fn.qrcode=function(b){
		...
		b=a.extend({},{
			...
			height:256,
			//这里是图片的高度和宽度
			imgWidth:b.width*3/10,
			imgHeight:b.height*3/10,
			typeNumber:-1,
			...
			if (b.src) {
				...
				logo.onload=function(){
					...
					b.imgWidth = b.imgWidth + padding * 2;
					b.imgHeight = b.imgHeight + padding * 2;
					...
					var left_x = padding - border + 1;
					var right_x = canvas.width - padding + border - 1;
					var top_y = padding - border + 1;
					var bottom_y = canvas.height - padding + border - 1;

					ctx.save();
					ctx.beginPath();
					//创建开始点
					ctx.moveTo(left_x + radius, top_y);
					//创建水平线
					ctx.lineTo(right_x - radius, top_y);
					//创建弧
					ctx.arcTo(right_x, top_y, right_x, top_y + radius, radius);

					//创建垂直线
					ctx.lineTo(right_x, bottom_y - radius);
					//创建弧
					ctx.arcTo(right_x, bottom_y,right_x - radius, bottom_y, radius);

					//创建水平线
					ctx.lineTo(left_x + radius, bottom_y);
					//创建弧
					ctx.arcTo(left_x, bottom_y, left_x, bottom_y - radius, radius);

					//创建垂直线
					ctx.lineTo(left_x, top_y + radius);
					//创建弧
					ctx.arcTo(left_x, top_y, left_x + top_y, top_y, radius);
					//进行绘制
					ctx.stroke();

					//画图
					ctx.drawImage(e, padding, padding, logo.width, logo.height);

					b.src = canvas.toDataURL();
					e.src = b.src;
				}
			}
