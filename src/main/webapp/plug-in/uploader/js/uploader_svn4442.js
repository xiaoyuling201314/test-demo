...
;(function (window, document) {
    ...
    let defaultConfigs = {
        ...
        previewWidth: 54,           // 压缩预览图的宽度，如果只设置宽度，则样式中高度等于宽度
    ...
    uploader.prototype = {
        ...
        buildUploadButton: function () {
            let html = '', configs = this.configs, uploadId = configs.id, fileName = configs.name;
            if (configs.isImage) {
                html += '<div class="upload-form fileIds clearfix">'
                    + '<input class="input-file ' + uploadId + '" type="file" id="files' + uploadId + '" accept="' + configs.accept + '" name="' + fileName + '"'
        ...
        bindHeadEvent: function () {
            ...
            $(".btn-select-file" + that.configs.id).on("click", function () {
                ...
                let html = '';
                html += '<input class="input-file ' + that.configs.id + '" type="file" id="files' + that.configs.id + '" accept="' + that.configs.accept + '" name="' + that.configs.name + '"'
                ...
                $(this).parent().children(".input-file").on("change", function (e) {
                    ...
                    var msg = that.updateFiles();//检测所有选中的文件是否符合标准
                    if (!msg) {
            ...
            $(".del_file").each(function (index, item) {
                $(item).click(function () {
                    ...
                    that.files.forEach(function (file, index2) {
                        that.configs.onRemove && that.configs.onRemove(file);
            ...
            if (maxCount === 1) {
                if (echoFileArr.length) {
                    echoFileArr.each(function (index, item) {
                        ...
                        $("#" + that.configs.id).append('<input type="hidden" name="delFileUrl" value="' + delFileUrl + '"/>');
                    });
                }
        ...
        renderFileListImage: function () {
            ...
            $("#" + this.configs.id).find("input[name=delFileUrl]").remove();
            this.files = [];
...
function showImageHtml(uploader, url, webRoot) {
    if (url) {
        ...
        let {id, maxCount, previewWidth, previewHeight} = uploader.configs;
        for (let i = 0; i < urlArr.length; i++) {
            html += '<li class="echo_data">';
            html += '<i class="del-img icon iconfont icon-close shanchu del_show_file" onclick="delShowImage(this,\'' + id + '\',\'' + urlArr[i] + '\')" data-url="' + urlArr[i] + '"></i>';
 ...
 * @param delFileUrl
 */
function delShowImage(o, id, delFileUrl) {
    $("#" + id).append('<input type="hidden" name="delFileUrl" value="' + delFileUrl + '"/>');
    $(o).closest("li").remove();
}
