	...
	if($.Datatype){
		...
		$.extend($.Datatype,{
			...
			*/
			...
			//车牌;
			"vehicleNum":/^[京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼使领A-Z]{1}[A-Z]{1}[A-Z0-9]{4}[A-Z0-9挂学警港澳]{1}$/,
			
			//手机号;
			// "telephoneNum":/^((13[0-9])|(14[5|7])|(15([0-3]|[5-9]))|(18[0,5-9]))\\d{8}$/,
			"telephoneNum":/^(1[0-9])\\d{9}$/,
			
			//固话;
			"phoneNum":/^(0\\d{2}-\\d{8}(-\\d{1,4})?)|(0\\d{3}-\\d{7,8}(-\\d{1,4})?)$/,
			...
			*/
			...
			//密码，大写字母、小写字母、数字和特殊字符中至少两种的组合
			"pw":/^(?![0-9]+$)(?![a-z]+$)(?![A-Z]+$)(?![@#$%^&*_<>,./~`:;"'=\\{\\}\\!\\(\\)\\+\\?\\|\\\\]+$)[0-9A-Za-z@#$%^&*_<>,./~`:;"'=\\{\\}\\!\\(\\)\\+\\?\\|\\\\]{8,16}$/
		});
	}else{
