...
/*	   toastr:(function(){
	   ...
	   loading:function(msg){
		  ...
		  var index = top.layer.msg('正在提交，请稍等...', {
			  icon: 16
			  ,shade: 0.01,
	 ...
	 //   },
	  ...
	  //获取字典标签
		...
		*    beformSubmit：执行提交操作前执行的方法
        ...
        //打开对话框(添加修改)
        //secondbtn,第二个按钮名称；sencondbtnfun 第二个按钮方法名称
        openSaveDialogTbtn: function(title,url,width,height,secondbtn,sencondbtnfun){
            var auto = true;//是否使用响应式，使用百分比时，应设置为false
            if(width.indexOf("%")>=0 || height.indexOf("%")>=0 ){
                auto =false;
            }
             top.layer.open({
                ...
                btn2:function(index, layero){
                	var iframeWin = layero.find('iframe')[0]; //得到弹出的窗口对象，执行窗口内iframe页的方法：iframeWin.method();
                 	var fun = iframeWin.contentWindow.eval(sencondbtnfun);//调用保存事件，在 弹出页内，需要定义save方法。处理保存事件。
                 	new fun();
                    return false;
                },
                cancel: function(index){
