var alignmentFns = {
    ...
    init: function(input, step, max, min, digit) {
        ...
        //添加左右加减号
        input.wrap("<div id = '" + input.attr('id') + "div' style = 'overflow:hidden;width:" + width + "px;height:" + height + "px;border: 1px solid #CCC;display:inline-block; margin-top: 6px;'></div>");
        input.before("<div id = '" + input.attr('id') + "l'  onselectstart = 'return false;' class='p-btn' style = '-moz-user-select:none;cursor:pointer;text-align:center;width:" + height + "px;height:" + height + "px;line-height:" + height + "px;border-right: 1px solid #CCC;float:left'>-</div>");
        input.after("<div id = '" + input.attr('id') + "r'  onselectstart = 'return false;' class='p-btn' style = '-moz-user-select:none;cursor:pointer;text-align:center;width:" + height + "px;height:" + height + "px;line-height:" + height + "px;border-left: 1px solid #CCC;float:right'> + </div>");
        //左右调用执行
