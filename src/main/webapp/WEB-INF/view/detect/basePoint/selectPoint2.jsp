...
<script type="text/javascript">
    ...
    function initPointSelect(){
        $("#point_select2").select2({
            ...
            width: "100%",
            ajax: {
                url: "${webRoot}/detect/basePoint/select2PointData",
                dataType: 'json',
                delay: 250,
                data: function(params) {//发送到服务器的数据
                    ...
                    return {
                        pointName: (!params.term ? '本地历史数据' : params.term), //后端取 key 它就是搜索关键字
                ...
                processResults: function(data, params) {// 后端返数据的根是 data，params就是上面的查寻参数
                    ...
                    if (!params.term) {
                        if ($.cookie("POINT_SEL")) {
                            return {
                                results: JSON.parse($.cookie("POINT_SEL")),
                                pagination: { //分页
     ...
     * 获取已选数据数组
     */
    function getPointSelect2Data() {
        var selData = $('#point_select2').select2('data');
        var POINT_SEL;
        if (!$.cookie("POINT_SEL")) {
            POINT_SEL = [];
            POINT_SEL.push({"id": selData[0].id, "name": selData[0].name, "number": 1});
        } else {
            POINT_SEL = JSON.parse($.cookie("POINT_SEL"));
            for (var i=0; i<POINT_SEL.length; i++) {
                if (POINT_SEL[i].id == selData[0].id) {
                    POINT_SEL[i].number = POINT_SEL[i].number + 1;
                    break;
                } else if (i == POINT_SEL.length-1) {
                    POINT_SEL.push({"id": selData[0].id, "name": selData[0].name, "number": 1});
                    break;
                }
            }
        }
        POINT_SEL.sort(function (a, b) {
        ...
        $.cookie("POINT_SEL", JSON.stringify(POINT_SEL));
        return $('#point_select2').select2('data');
    }

    /**
     * 清除选择项
     */
    function cleanPointSelected() {
        $("#point_select2").select2('val', "");
        $("#point_select2").select2('data', null);
        $("#point_select2").empty();
    }
</script>
