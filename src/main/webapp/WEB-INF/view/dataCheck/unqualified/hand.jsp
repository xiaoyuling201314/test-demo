...
<%--<div class="cs-deliver-dian cs-hide">
    <form action="${webRoot}/dataCheck/unqualified/save.do" id="saveForm2" method="post" enctype="multipart/form-data">
        <input class="cs-hide" type="text" name="treatment.id" value="${checkResult.dutId}"/>
        <input class="cs-hide" type="text" name="treatment.dealMethod" value="0"/>
        <input class="cs-hide" type="text" name="treatment.checkRecordingId" value="${checkResult.id}"/>
        <input class="cs-hide" type="text" name="treatment.dealType" value="1"/>
        <input class="cs-hide" type="text" name="treatment.recheckResult" value="处理中"/>
        <table class="cs-add-new">
            <tr>
                <td class="cs-name"><i class="cs-mred">*</i>接收报告时间：</td>
                <td class="cs-in-style">
                    <input name="treatment.sendDate" class="cs-time" type="text"
                           value='<fmt:formatDate value='<%=new Date() %>' pattern='yyyy-MM-dd HH:mm:ss' />'
                           onClick="WdatePicker({startDate:'%y-%M-%d %H:%m:%s',dateFmt:'yyyy-MM-dd HH:mm:ss',alwaysUseStartDate:true})" datatype="*"
                           nullmsg="请选择送检时间"/>
                </td>
                <td class="cs-name"><i class="cs-mred">*</i>处理时间：</td>
                <td class="cs-in-style " style="width:250px;">
                ...
                <td class="cs-name"><i class="cs-mred">*</i>送检人签名：</td>
                <td class="cs-in-style">
                    <input name="treatment.spersonName" datatype="*" nullmsg="请输入送检人签名" type="text" value=""/>
                </td>
            </tr>
            <tr>
                <td class="cs-name">复检机构：</td>
                <td class="cs-in-style">
                    <input name="treatment.recheckDepart" id="departName">
                </td>
                <td class="cs-name ">取证信息附件：</td>
                <td>
                    <div style="width:200px;">
                        <input type="file" id="myFile2" name="dealImgurlFile" multiple="multiple" accept="image/jpeg"/>
                    </div>
                </td>
            </tr>
            <tr>
                ...
                <td class="cs-in-style" colspan="5">
                    <textarea name="treatment.remark" id="" cols="30" rows="10" class="cs-text-for"></textarea>
                </td>
            </tr>
        </table>
        <div class="cs-hd"></div>
        <div class="cs-alert-form-btn">
            <a href="javascript:;" class="cs-menu-btn" id="btnSave2"><i class="icon iconfont icon-save"></i>保存</a>
            <a href="${webRoot}/dataCheck/unqualified/list" class="cs-menu-btn">
    ...
    $(function () {
        ...
        $("#saveForm").Validform({
            ...
            beforeSubmit: function () {
                ...
                let files = upload.files;
                if (files.length > 0) {
                    // formData.append(upload.configs.name,files[0]);
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data && data.success) {
                            self.location = "${webRoot}/dataCheck/unqualified/list.do";
                        } else {
