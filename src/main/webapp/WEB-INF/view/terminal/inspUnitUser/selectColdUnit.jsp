...
<!-- 选择检测标准 - 下拉框 -->
<select id="coldUnitSelect" class="js-data-example-ajax"></select>
<script type="text/javascript" src="${webRoot}/js/jquery.cookie.js"></script>
<script type="text/javascript">
	$("#coldUnitSelect").select2({
		placeholder: '--请选择--',
		language: {
		...
		width: "100%",
		ajax: {
			url: "${webRoot}/cold/unit/select2RegData",
			dataType: 'json',
			delay: 250,
			data: function(params) {//发送到服务器的数据
				...
				return {
                    regName: params.term, //后端取 key 它就是搜索关键字
	...
	function getSelect2Data() {
        var selData = $('#coldUnitSelect').select2('data');
        // //录入数据不保存到历史
        // if (selData[0].id > 0) {
            var STANDARD_SEL;
        ...
        return $('#coldUnitSelect').select2('data');
    }
    ...
    function cleanSelected() {
        $("#select2-coldUnitSelect-container").text("--请选择--");
        $("#coldUnitSelect").select2('val', "");
        $("#coldUnitSelect").select2('data', null);
        $("#coldUnitSelect").empty();
    }
</script>
