...
<div class="cs-col-lg clearfix">
    ...
    <ol class="cs-breadcrumb">
        ...
        <li class="cs-b-active cs-fl">充值活动
        </li>
    </ol>
    <!-- 面包屑导航栏  结束-->
...
var chakanObj=null;
    for (var i = 0; i < childBtnMenu.length; i++) {
        if (childBtnMenu[i].operationCode == "1459-1") {
            var html = '<a class="cs-menu-btn" href="${webRoot}/activities/edit.do" ><i class="' + childBtnMenu[i].functionIcon + '"></i>新增</a>';
            $("#showBtn").append(html);
        } else if (childBtnMenu[i].operationCode == "1459-8") {
            edit = 1;
            editObj = childBtnMenu[i];
        } else if (childBtnMenu[i].operationCode == "1459-2") {//删除
            deletes = 1;
            deleteObj = childBtnMenu[i];
        }else if (childBtnMenu[i].operationCode == "1459-5") {//立即生效
            openNow = 1;
            openNowObj = childBtnMenu[i];
        }else if (childBtnMenu[i].operationCode == "1459-6") {//立即终止
            closeNow = 1;
            closeNowObj = childBtnMenu[i];
        }else if (childBtnMenu[i].operationCode == "1459-7") {//查看操作日志
        	log = 1;
        	logObj = childBtnMenu[i];
        }else if (childBtnMenu[i].operationCode == "1459-10") {//查看
        	chakan = 1;
    ...
    var op = {
        ...
        tableAction: "${webRoot}/activities/datagrid.do",	//加载数据地址
        parameter: [		//列表拼接参数
            {
                columnCode: "theme",
                columnName: "活动主题",
        ...
        funBtns: [
            {
                ...
                style: editObj,
                action: function (id) {
                	self.location="${webRoot}/activities/edit.do?id="+id;
                }
            },
            {
                show: chakan,
            ...
            },
            {
                ...
                style: logObj,
                action: function (id) {
                    var src = '${webRoot}/activitiesLog/list.do?actId='+id;
	    			showMbIframe(src);
                }
            }
        ]/* ,
		    	...
		    	$(".rowTr").each(function(){
			    	for(var i=0;i<obj.length;i++){
				    		if($(this).attr("data-rowId") == obj[i].id){
				    			...
				    			if(status==0&&checked==1){//活动未开启 关闭按钮隐藏
				    				$(this).find(".1459-2").hide();
				    			}else if(status==0&&checked==0){//未审核  
				    				$(this).find(".1459-10").hide();
				    				$(this).find(".1459-5").hide();
					    			$(this).find(".1459-6").hide();
				    			} else if(status==1){//进行中 开启按钮隐藏
				    				$(this).find(".1459-5").hide();
				    				$(this).find(".1459-2").hide();
				    				$(this).find(".1459-8").hide();
    ...
    function satrtOffer(id) {
		  ...
		  $.ajax({
	            type: "POST",
	            url: "${webRoot}/activities/startOffer.do?id="+id,
	            data: {},
	            dataType: "json",
    ...
    function stopOffer(id) {
	  ...
	  $.ajax({
           type: "POST",
           url: "${webRoot}/activities/stopCron.do?id="+id,
           data: {},
           dataType: "json",
           success: function (data) {
               if (data && data.success) {
    ...
    function deleteData() {
        $.ajax({
            type: "POST",
            url: "${webRoot}/activities/delete.do",
