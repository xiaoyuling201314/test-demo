...
　　	}
    ...
    .food-list-table{
      ...
      .zz-title-bg{
            ...
            color: #fff;
            font-size: 18px;
            background:#007eea url('img/top.jpg') center center no-repeat;
            background-size: 100%;
        ...
        <div class="zz-cont-box">
            <div class="zz-choose  col-md-12 col-sm-12">
                 <div class="col-md-12 col-sm-12" style="padding-right: 0;">
                    ...
                    <input type="hidden" name="reportPrice" value="${bean.reportPrice}">
                     ...
                     <div class="pull-left cs-all-ps">
	                    ...
	                    <div class="cs-input-box">
		                     <input type="hidden"  name="labelId" class="cs-input-id" />
							 <input type="text" id="inputDataRegLabel" class="cs-down-input inputData" autocomplete="off" placeholder="请输入首字母或全拼查找单位"/>
						 </div>
						 <div id="divBtn" class="cs-check-down" style="display:none;">
		                      <ul id="mySelectLabelId">
		                   	  </ul>
	                    </div>
	                    </div>
	                    <c:choose>
	                    	<c:when test="${isConfig==1}">
			                    <span class="icon iconfont icon-xiugai zz-xiugai" style="margin-left: 10px;" onclick="queryLabelById('edit');"></span>
	                    	</c:when>
	                    	<c:when test="${isConfig==0}">
		                    	<span class="icon iconfont icon-zengjia zz-xiugai" style="margin-left: 10px;" onclick="queryLabelById('add');"></span>
	                    	</c:when>
	                    </c:choose>
                     </div>
...
var webRoot="${webRoot}";
    $(function(){
    	...
    	//默认加载历史纪录
    	queryHistory(true,true,true,true);
        /* center modal */
        ...
        $(document).on('click', '#mySelectLabelId li', function () {
        	$("input[name=labelId]").val($(this).attr("data-id"));
        	$("#inputDataRegLabel").val($(this).attr("data-requesterName"));
      ...
      //点击结算：验证必填项        
        $(document).on("click","#_settlement",function() {
        	 if(requestList.length==0){
        		...
        		var inspectionMoney=parseFloat($("input[name='inspectionFee']").val());
        		var price=parseFloat("${bean.reportPrice}");
        		var number=requestList.length-1;
        		$("#inspection-money").html(inspectionMoney.toFixed(2)+"元");
        		if(number>=1){
            		$("#report-money").html(price+"元 *"+number);
            		$("#showReportMoney").removeClass("cs-hide");
        		}else{
       	 			$("#showReportMoney").addClass("cs-hide");
        		}
        		var totalMoney=price*number+inspectionMoney;
        		$("#total-money").html(totalMoney.toFixed(2)+"元");
...
//	            $("#_item_ul input:checkbox:checked").iCheck('uncheck');
        ...
        $(document).on("click","._bill_confirm_btn",function() {
            ...
            $("input[name=requestList]").val(JSON.stringify(requestList));
            if($("input[name=labelId]").val()=="0" && requestList.length>1){
            	$("#inputDataRegLabel").val(requestList.length+"个委托单位");
            }
            $.ajax({
                type: "POST",
                url: "${webRoot}/order/saveMulti",
  ...
  //编辑委托单位信息
	function queryLabelById(type){
		 showMbIframe("${webRoot}/inspUnitUser/queryLabelById?labelId="+$("input[name=labelId]").val()+"&operatorType="+type);
		 $(".zz-content").hide(); 
	}
	/* //批量保存ajax请求
