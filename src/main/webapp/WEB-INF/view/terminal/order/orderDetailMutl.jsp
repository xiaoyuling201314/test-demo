                ...
                <div class="cs-add-pad cs-input-bg">
                    ...
                    <table class="cs-form-table cs-form-table-he" >
                        ...
                        <tr>
                            ...
                            <td id ="samplingNo" style="display: none;">${sampling.samplingNo}</td>
                            <td id ="regName" style="display: none;">${sampling.regName}</td>
                            ...
                            <td class="cs-in-style" style=" padding-left:10px;" colspan="3">
                                <c:if test="${!empty sampling && !empty sampling.samplingDate}">
                                    <fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss" value="${sampling.samplingDate}" />
                                </c:if>
                            </td>
                        ...
                        <tr>
                            <td class="cs-name" style="width:160px;">委托单位：</td>
                            <td class="cs-in-style" style=" padding-left:10px;">
                                <c:if test="${!empty sampling}">${sampling.regName}</c:if>
                            </td>
                        ...
                        <tr>
                            ...
                            <td class="cs-in-style" style=" padding-left:10px;" colspan="2">
                                <c:if test="${!empty sampling}">${sampling.param3}</c:if>
                            </td>
                        </tr>
                        <c:if test="${!empty sampling.inspectionId}">
                        <tr>
                            <td class="cs-name" style="width:160px;">送检单位：</td>
                            <td class="cs-in-style" style=" padding-left:10px;" colspan="3">
                                <c:if test="${!empty sampling}">${sampling.inspectionCompany}</c:if>
                            </td>
                        </tr>
                        </c:if>
                        </tbody>
...
<%--                     <h3>委托单位列表</h3>
                    <table class="cs-form-table cs-form-table-he">
                     ...
                     <c:forEach var="obj" items="${reList}">
                        <tr>
                           <td class="cs-name" style="width:160px;">委托单位：</td>
                           <td class="cs-in-style" style="width:34%; padding-left:10px;">${obj.requesterName}</td>
                           <td class="cs-name" style="width:160px;">单位电话：</td>
                           <td class="cs-in-style">${obj.param1}</td>
                        </tr> 
                        </c:forEach>
                    </table> --%>
                    <h3>付款信息</h3>
                    <table class="cs-form-table cs-form-table-he" >
                        ...
                        <tr>
                            ...
                            <td class="cs-in-style" style="width:34%; padding-left:10px;">
                                <c:if test="${!empty sampling && !empty sampling.inspectionFee}">￥${sampling.inspectionFee}</c:if>
                            </td>
                            <td class="cs-name" style="width:160px;">付款状态：</td>
                            <td class="cs-in-style" style=" padding-left:10px;">
                                <c:if test="${!empty sampling}">
                                    <c:choose>
                                        ...
                                        <c:when test="${sampling.orderStatus eq 3}">取消订单
...
//                 	return;
                    $("#mdataList .rowTr").each(function (index, value) {
                        if (id == $(this).attr("data-rowid")) {
                        	...
                        	getRegName($("#id").text(),reportNum,$(this).find(".collectCode").text());
            ...
            onload: function(){
              ...
              //计算合并行数
                var reportCodes = {};   //同一报告码，打印按钮合并行数(已打印)
                var sampleTime = {};   //同一次送样，打印按钮合并行数(未打印)
                $(datagridOption.obj).each(function(index, value){
                    if (value.sampleTubeTime) {   //已收样
                    	 if (!sampleTime[value.sampleTubeTime]){
                             sampleTime[value.sampleTubeTime] = 1;
                         } else {
                             sampleTime[value.sampleTubeTime] = sampleTime[value.sampleTubeTime]+1;
                         }
                    }
                });
				console.log(sampleTime);
				//合并打印按钮  按收样批次进行打印，同一批次只有所有样品检测完成才能打印报告 update by xiaoyl 2020-2-12
                $("#mdataList tr").each(function (index,value) {
                    var sysj = $(value).find(".sampleTubeTime").text();
                    if(sysj !=null&&sysj!=""){
                    	$(value).find(".1443-6").show();
                    }else{
                    	 $(value).find("td:last-child").children("span:last-child").hide();
                    }
                    
                       //合并打印按钮
						var rcn = sampleTime[sysj];  //合并行数
                        delete sampleTime[sysj];
                        var dtr = $(this);
                        var isShow=0;//已出检测结果数量
                        for(var j=0;j<rcn;j++){
                            if (j==0){
                            ...
                            if(dtr.find(".conclusion").text()){
                        		isShow+=1;
                        	}
                        }
                        if(isShow==rcn){//判断已出结果样品是否等于收样数量
                        	$(this).find(".1443-2").show();
                        }
                    //判断抽样明细是否已被仪器接收
                    if(showPurchaseNumber==1 && $(value).find(".recevieDevice").html()!=""){
                        $(value).find(".recevieDevice").html('<i class="icon iconfont icon-yiqi" style="margin-left: 4px;" title="'+$(value).find(".recevieDevice").html()+'"></i>');
                    }
                });
            }
		};
...
layer.open({
    ...
    btnAlign: 'c',
    area: ['670px', '410px'],
    content: '${webRoot}/requester/unit/listview?samplingId='+samplingId+'&filterDelete=1',
     btn: ['确定','关闭'],
...
function printProof(collectCode){
    ...
    var orderNo = "${sampling.samplingNo}";  //订单号
    ...
    var wtdw ="${sampling.samplingUsername}";  //委托单位名称
...
/*     if (wtdwArr && wtdwArr.length>0){
    ...
    var reportNum;//取报告码
	 $("#mdataList .rowTr").each(function (index, value) {
		 if($(this).find(".collectCode").text()==collectCode){//同一收样批次
			 var item_name=$(this).find("td:eq(3)").text();//检测项目名称
// 			 reportNum = $(this).find("td:eq(9)").text();
			 collectTime=$(this).find("td:eq(5)").text();
			 _hight = _hight + _line_hight;
			    sendMessage+="TEXT "+_text_left+","+_hight+",\"TSS24.BF2\",0,1,1,\""+$(this).find("td:eq(2)").text()+"\"\r\n" +
