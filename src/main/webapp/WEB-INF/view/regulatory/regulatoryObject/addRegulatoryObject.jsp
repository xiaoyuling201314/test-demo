...
<body>
	<div class="cs-maintab">
			...
			<div class="cs-search-box cs-fr">
				<form>
					<div class=" cs-fr cs-ac ">
						...
						<a class="cs-menu-btn" href="javascript:" onclick="self.location.href='${webRoot}/regulatory/regulatoryObject/list.do?regTypeCode=${regulatoryType.regTypeCode}'"><i class="icon iconfont icon-fanhui"></i>返回</a>
		...
		<form id="regForm" action="${webRoot}/regulatory/regulatoryObject/save.do" method="post">
			...
			<div class="cs-base-detail">
				<div class="cs-content2">
					...
					<table class="cs-add-new">
						<tr>
							<td class="cs-name"><i class="cs-mred">*</i>企业名称：</td>
							...
							<td class="cs-name"><i class="cs-mred">*</i>企业类型：</td>
							<td class="cs-in-style">
								<select name="regType" datatype="*" nullmsg="请选择企业类型" errormsg="请选择企业类型">
									<c:forEach items="${regulatoryTypes}" var="rtype">
										<option value="${rtype.id}" <c:if test="${!empty regulatoryObject && rtype.id == regulatoryType.id}">selected="selected"</c:if>>${rtype.regType}</option>
									</c:forEach>
								</select>
							</td>
						...
						<tr>
							...
							<td class="cs-in-style"><input type="text" name="fax" <c:if test="${!empty regulatoryObject}">value="${regulatoryObject.fax}"</c:if>/></td>
							<td class="cs-name">邮编：</td>
							<td class="cs-in-style"><input type="text" name="post" <c:if test="${!empty regulatoryObject}">value="${regulatoryObject.post}"</c:if>/></td>
							<td class="cs-name">通讯地址：</td>
							<td class="cs-in-style"><input type="text" name="regAddress" readonly="readonly" <c:if test="${!empty regulatoryObject}">value="${regulatoryObject.regAddress}"</c:if>/></td>	
						</tr>
						<tr>
	...
	var regFormValidform = $("#regForm").Validform({
		callback:function(data){
			...
			if(data && data.success){
				...
				if(saveType==1){
					self.location = '${webRoot}/regulatory/regulatoryObject/goAddRegulatoryObject.do?regTypeId='+$("select[name='regType']").val();
				}else{
					<%--self.location = '${webRoot}/regulatory/regulatoryObject/goAddRegulatoryObject.do?id='+data.obj.id+'&regTypeId='+$("select[name='regType']").val();--%>
					self.location = '${webRoot}/regulatory/regulatoryObject/list';
				}
