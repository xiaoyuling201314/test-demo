		...
		<div class="wrapper">
			<form class="" id="saveForm" enctype="multipart/form-data">
				<div class="is-flex">
					...
					<div class="">
						<p>
							<b><font color="#000000" size="4">经营户导入要求：</font></b>
						</p>
						<p>
							1、必须是EXCEL格式。 <a class="text-primary cs-link"
											 href="${webRoot}/js/business_importtemplate.xlsx">下载模版</a>
				...
				<%--<div class="form-group col-sm-12 col-md-12">--%>
					...
					<%--<div class="col-sm-8">--%>
						<%--<p><b><font color="#000000" size="4">经营户批量导入注意事项：</font></b></p>--%>
						<%--<p><font color="#000000">1、文件必须是EXCEL格式。--%>
						<%--<a href="${webRoot}/js/business_importtemplate.xlsx" style="text-decotation:underline;color:#FF0000;">点击此处下载模版</a></font>--%>
						<%--<br />--%>
					...
					<div  class="">
						...
						<c:choose>
							<c:when test="${isNewMenu=='N'}">
								<c:if test="${type==3}">
									<a class="btn btn-default" href="${webRoot}/dataCheck/recording/importList?importType=3"><i class="iconfont icon-fanhui text-primary"></i> 返回</a>
								</c:if>
								<c:if test="${type!=3}">
									<a class="btn btn-default" href="${webRoot}/ledger/regulatoryObject/list.do?regTypeCode=0"><i class="iconfont icon-fanhui text-primary"></i> 返回</a>
								</c:if>
							</c:when>
							<c:when test="${isNewMenu=='Y'}">
								<a class="btn btn-default" href="javascript:history.back(-1);"><i class="iconfont icon-fanhui text-primary"></i> 返回</a>
							</c:when>
						</c:choose>
			...
			if($("#departId").val()!=""){
				if($("#kv-explorer").val()!=""){
					...
					$("#myModal-mid4").modal("show");
		  			...
		  			$.ajax({
		  		        type: "POST",
		  		        url: '${webRoot}'+"/regulatory/business/importData",
		  		        data: formData,
		  		        ...
		  		        dataType: "json",
		  		      	success: function(data){
		  		      		...
		  		      		if(data.success==true){
			  		      		...
			  		      		if('${type}'){
			  		      			window.location.href="${webRoot}/dataCheck/recording/importList?importType=3";
		  		      			}else{
		  		      				<%--if(data.msg){--%>
		  		      				<%--	window.location.href="${webRoot}/resources/business/"+data.msg;--%>
		  		      				<%--}else {--%>
			  		      				$("#myModalLabel").text(data.msg+",请前往【数据中心】-->【数据导入】中查看原因");
					  		      		$("#confirm-sure").modal('toggle');
									// }
								}
		  		      		}
