    ...
    <div class="all-content new-type">
        <div class="ui-header first-header">
            <h4>
                ...
                ${regName}
                <a href="${webRoot}/iRegulatory/Object/busAddApp?regId=${regId}&regName=${regName}&userToken=${userToken}" class="addSample check-detail">
        ...
        <div class="stats-search stats-search2 is-flex">
            ...
            <input id="keywords" class="insert-company" type="text" placeholder="请输入档口编号">
        ...
        <div id="pull-refresh" class="refresh-all" ontouchstart>
            <div id="busList" class="invoice-list clearfix" ></div>
        </div>
    </div>

    <div id="busTemplete" style="display: none;">
            ...
            <div class="law-box risk-check is-flex">
                <div class="round-check is-flex">
                    ...
                    <div class="round-info is-flex">
                        ...
                        <div><b class="opeShopCode"></b></div>
                        <div class=" text-left invoice-info">
                            <p>
                                摊位名称：<span class="text-muted opeShopName"></span>
                            </p>
                            <p>经营者：<span class="text-muted opeName"></span></p>
                        </div>
                    </div>
                    <div class="list-btn is-flex">
                        <a class="editBus"><i class="iconfont icon-xiugai text-primary"></i></a>
                    </div>
                </div>
            </div>
        </div>
                ...
                if (data.success) {
                    if (data.obj.results.length > 0){
                        ...
                        for (var i = 0; i < data.obj.results.length; i++) {
                            ...
                            var busIt = $("#busTemplete").clone();
                            busIt.find(".num-box").text((i + 1));
                            busIt.find(".opeShopCode").text(item.opeShopCode);
                            busIt.find(".opeShopName").text(item.opeShopName);
                            busIt.find(".opeName").text(item.opeName);
                            busIt.find(".editBus").attr("href","${webRoot}/iRegulatory/Object/busAddApp?id="+item.id+"&regId="+regId+"&regName="+encodeURI(regName)+"&userToken=${userToken}");
                            $("#busList").append(busIt.html());
                        }

                    } else {
                        $("#busList").html('<div class="invoice-li invoice-control weui-cell_swiped"><div class="law-box risk-check is-flex"><div class="round-check is-flex">暂无数据</div></div></div>');
                    }
                } else {
                    $.toast(msg, "cancel");
                }
