...
<html>
	...
	<body>
          ...
          <!-- 面包屑导航栏  开始-->
            <ol class="cs-breadcrumb">
              ...
              <li class="cs-b-active cs-fl">进货台账统计
              </li>
            </ol>
     ...
     <div id="second" class="cs-echart" style="width: 100%;height:380px; display:inline-block;"></div>
		...
		<div style="padding-bottom:50px;">
             <table class="cs-table cs-table-hover table-striped cs-tablesorter" id="table">
                <thead>
                  <tr>
                    <th class="cs-header columnHeading" style="width: 50px" onclick="datagridUtil.orderBy2('table',0,'int')" >序号</th>
             ...
             <table class="cs-table cs-table-hover table-striped cs-tablesorter" id="table1">
                <thead>
                  <tr>
                    <th class="cs-header columnHeading" style="width: 50px" onclick="datagridUtil.orderBy2('table1',0,'int')" >序号</th>
 ...
 <!-- Button to trigger modal -->
	...
	function loadData() {
		...
		$.ajax({
	        type: "POST",
	        url: '${webRoot}'+"//ledger/stock/loadRegReport.do",
	        ...
	        success: function(data){
	        	if(data && data.success){
	        		...
	        		var allyiwanc=0;
	        		var allwwc=0;
	        		console.log(data.attributes.scstock);
	        		$("#table tbody").html("");
	        		$("#table1 tbody").html("");
	        		if(data.obj){
	        			if(data.attributes.scstock==1){//市场录入数据
		        			...
		        			bigbang1();
		        			 datagridUtil.orderBy2('table1',4,'int'); 
			        			datagridUtil.orderBy2('table1',5,'float');
			        			datagridUtil.orderBy2('table1',5,'float');
	        			}else{
	        				$("#table").show();
	        				$("#second").hide();
	        			...
	        			$.each(data.obj, function(k, v) {
	        				...
	        				var bigbang="<tr>"
	                            ...
	                            +"<td>"+v.allCount+"</td>"
	                            +"<td>"+v.yiwanc+"</td>"
	                            +'<td><a href="'+'${webRoot}'+'/ledger/business/ledgerList.do?regId='+v.reg_id+'">'+wwc+'</a></td>'
	                            +"<td>"+wcl+"%</td>"
	                            		+"</tr>"
