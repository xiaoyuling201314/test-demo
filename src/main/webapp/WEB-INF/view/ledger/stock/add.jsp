...
<body>
	...
	<div class="cs-tb-box">
		<div class="cs-base-detail">
			<div class="cs-content2 clearfix">
				<form id="stockForm" action="${webRoot}/ledger/stock/save.do" method="post" enctype="multipart/form-data">
					...
					<div class="cs-add-new cs-add-pad">
						<ul class="cs-ul-style clearfix">
							...
							<li class="cs-in-style cs-md-w"><select class="js-select2-tags" name="regId" id="regId" onchange="changeReg()" datatype="*" nullmsg="请选择被检单位">
						...
						<div style="padding-bottom: 5px; clear: both;">
							<ul class=" cs-gh clearfix">
								<li class="cs-name cs-sm-w"><i class="cs-mred">*</i>选择食品：</li>
								<li class="cs-in-style cs-sm-w  cs-select-search" style="width: 40%;">
									<%-- <input type="text"  id="foodName"  name="foodName"  value="${bean.foodName }" onclick="getFood();"datatype="*" nullmsg=""   readonly="readonly"/> --%> <input type="text" id="foodName"  onclick="getFoodHistory();" autocomplete="off" class="cs-select-down" >
									<div class="cs-down-box cs-hide" id="foodBox">
										<input type="hidden" id="food">
										<ul id="foodList">
										</ul>
									</div> <span class="text-muted"> &nbsp; &nbsp;(可以选择多个食品) </span>
								</li>

							</ul>
						</div>
						<div class="food-item-list" id="fooditems">
							<%-- 	<table class="registerform"style="width: 88%;margin: 0 auto;"><tr style="background:#f1f1f1;">
					<td class="cs-name cs-sm-w">食品名称：</td><td class="cs-in-style cs-md-w"><input type="text"id="foodName"name="foodName"onclick="getFoodHistory();"autocomplete="off"class="cs-select-down"value="${bean.foodName }"datatype="*"nullmsg=""></td>
					<td class="cs-name cs-sm-w">进货数量：</td><td class="cs-in-style cs-md-w food-num"><input type="text"value="${bean.stockCount }"autocomplete="off"name="stockCount"id="stockCount"datatype="*"nullmsg=""/></td>
					<td class="cs-name cs-sm-w">规格单位：</td><td class="cs-in-style cs-md-w  cs-select-search food-num"><input class="cs-select-down"type="text"name="size"id="size"autocomplete="off"><div class="cs-down-box cs-hide"><ul id="sizeList"><li>KG</li><li>件</li><li>箱</li><li>只</li><li>个</li><li>头</li></ul></div>
					<a class="icon iconfont icon-shanchu text-danger" style="font-size: 20px;margin-left: 40px;"></a></td></tr></table> --%>
						</div>
						<ul class="cs-gh clearfix">
							<li class="cs-name cs-sm-w">进货日期：</li>
							<li class="cs-in-style cs-md-w"><input class="cs-time" type="text" name="stockDate" class="inputxt" onClick="WdatePicker({maxDate:'%y-%M-%d',dateFmt:'yyyy-MM-dd',alwaysUseStartDate:true})" <c:if test="${empty bean.stockDate }">value="<fmt:formatDate value='<%=new Date()%>' pattern='yyyy-MM-dd'/>"</c:if> <c:if test="${!empty bean.stockDate }">value="<fmt:formatDate  value='${bean.stockDate }' pattern='yyyy-MM-dd'/>" </c:if> /></li>
							...
							<li class="cs-in-style cs-md-w cs-select-search">
								...
								</div> --%> <select class="easyui-combobox ObjList" name="param1" panelHeight="auto" panelMaxHeight="260px" style="height: 29px; width: 200px;">
							</select>
							</li>
							<li class="cs-name cs-sm-w">供货档口：</li>
							<li class="cs-in-style cs-md-w cs-select-search">
								<%-- <input type="text" value="${bean.supplier }" readonly="readonly" onclick="getOpe();"  name="supplier" id="supplier"  autocomplete="off"/> --%> <%-- <input type="text" class="cs-select-down" autocomplete="off"  name="supplier" onclick="getObjHistory(0);" id="supplier" value="${bean.supplier }"> --%> <select class="easyui-combobox OpeList" name="supplier" panelHeight="auto" panelMaxHeight="260px" style="height: 29px; width: 200px;">
							</select>
							</li>
							...
							<li class="cs-in-style cs-md-w"><input class="cs-time" type="text" name="productionDate" id="productionDate" class="inputxt" onClick="WdatePicker({maxDate:'%y-%M-%d %H:%m:%s',dateFmt:'yyyy-MM-dd',alwaysUseStartDate:true})" <c:if test="${empty bean.productionDate }">value="<fmt:formatDate value='<%=new Date()%>' pattern='yyyy-MM-dd'/>"</c:if> <c:if test="${!empty bean.productionDate }">value="<fmt:formatDate  value='${bean.productionDate }' pattern='yyyy-MM-dd'/>" </c:if> /></li>
						</ul>
						<ul class=" cs-gh clearfix">
						...
						<!-- 文件存储盒子 Img1 -->
						<!-- <div style="display: none" id="fileBox1">
						</div>
						<div  id="fileBox2">
						</div>
						<div style="display: none" id="fileBox3">
						</div> -->
						<ul class=" cs-gh tz-gallery clearfix">
							...
							<span class="cs-inline-blcok clearfix">
									<div class="uploader-box" id="img2"></div>
									<label   for="browerfile2" class="file cs-fl" id="Img2-add" style="display: "> <input type="file" id="browerfile2" name="Img2" accept="image/*" multiple></label>
							...
							<span class="cs-inline-blcok clearfix" >
								<div class="uploader-box" id="img3"></div>
								<label  for="browerfile3" class="file cs-fl" id="Img3-add" style="display: "> <input type="file" id="browerfile3" name="Img3" accept="image/*" multiple></label>
							...
							<li class="cs-in-style cs-fl  cs-md-w">
							<span class="cs-inline-blcok clearfix">
									<div class="uploader-box" id="img1"></div>
									<label  for="browerfile1" class="file cs-fl" id="Img1-add" style="display: "> <input type="file" id="browerfile1" name="Img1" accept="image/*" multiple></label>
							</span>
	...
	<div class="cs-alert-form-btn clearfix">
		<c:if test="${showType!=1}">
			<a href="javascript:" class="cs-menu-btn" id="stockBtnSave"><i class="icon iconfont icon-save"></i>保存</a>
		</c:if>
		<c:if test="${empty bean.id}">
			<a href="javascript:" class="cs-menu-btn" id="stockBtnSaveself"><i class="icon iconfont icon-save"></i>保存并新增</a>
		</c:if>
	...
	//保存数据		saveStock(formData);
		function saveStock(formData) {
			...
			if(!param1){
				$("#confirm-warnning .tips").text("请选择来源市场!");
				$("#confirm-warnning").modal('toggle');
				return;
			}
				$("#stockForm").submit();
...
//校验样品是否存在
	...
	function test(foodName) {
		if(foodName){
			var res=checkfooditems(foodName);
			if(!res){
				$("#confirm-warnning .tips").text("该食品已选择!");
				$("#confirm-warnning").modal('toggle');
			}else{
			 var fooditemsHtml=' 	<table class="registerform"style="width: 88%;margin: 0 auto;"><tr style="background:#f1f1f1;">'
					+'<td class="cs-name cs-sm-w"><i class="cs-mred">*</i>食品名称：</td><td class="cs-in-style cs-md-w"><input type="text"id="foodName"readonly="readonly" name="foodName"value="'+foodName+'"autocomplete="off"class="cs-select-down"datatype="*"nullmsg=""></td>'
					+'<td class="cs-name cs-sm-w"><i class="cs-mred">*</i>进货数量：</td><td class="cs-in-style cs-md-w food-num"><input type="text"autocomplete="off"  name="stockCounts"id="stockCount" onkeyup="clearNoNum(this)"   datatype="*"nullmsg=""/></td>'
					+'<td class="cs-name cs-sm-w"><i class="cs-mred">*</i>规格单位：</td><td class="cs-in-style cs-md-w  cs-select-search food-num"><input class="cs-select-down"type="text"name="size"id="size" value="KG" autocomplete="off"><div class="cs-down-box cs-hide" style="width:100px;"><ul id="sizeList" class="sizeList"><li>KG</li><li>件</li><li>箱</li><li>只</li><li>个</li><li>头</li></ul></div>'
					+'<a class="icon iconfont icon-shanchu text-danger" style="font-size: 20px;margin-left: 40px;" value="'+foodName+'"></a></td></tr></table>'
			
			$("#fooditems").append(fooditemsHtml);
					$('.food-items').show();
	...
	function checkfooditems(foodName) {
		var res=true;
	...
	function delfooditems(foodName) {
		var res=true;
		for (var i = 0; i < fooditems.length; i++) {
			var food = fooditems[i];
			if(food==foodName){
...
function change(img){
	...
	var allFiles = imgFile.files;
	 var formData = new FormData($('#stockForm')[0]);
	 var fileObj = formData.get("Img1");
	console.log(fileObj);
	 arr3.push(imgFile.files);
	console.log(arr3);
	for(var i=0; i < allFiles.length; i++){
		var img = allFiles[i];
		$("img").remove();
