			...
			<div class="cs-search-box cs-fr">
				<form>
					<div class=" cs-fr cs-ac ">
						<a class="cs-menu-btn" href="javascript:" onclick="save();"><i class="icon iconfont icon-save"></i>保存</a> <a class="cs-menu-btn" href="javascript:;" onclick="saveself();"><i class="icon iconfont icon-save"></i>保存并新增</a>
						 <a class="cs-menu-btn" href="javascript:" onclick="self.location.href='${webRoot}/ledger/regulatoryObject/ledgerList.do?regTypeCode=${regulatoryType.regTypeCode}'"><i class="icon iconfont icon-fanhui"></i>返回</a>
					...
					<ul class=" cs-gh cs-add-new clearfix">
						...
						<li class="cs-in-style cs-fl  cs-md-w">
							<div class="cs-all-ps">
								<div class="cs-input-box">
									<input type="text" name="regName" id="regName" datatype="*" nullmsg="请输入市场名称" errormsg="请输入企业名称" <c:if test="${!empty regulatoryObject}">value="${regulatoryObject.regName}"</c:if> autocomplete="off" />
									<div class="cs-map" data-toggle="modal" data-target="#positionModal" onclick="setMapLocation('${regulatoryObject.placeX}','${regulatoryObject.placeY}');">
						...
						<li class="cs-in-style cs-md-w  cs-add-new"><select name="regType" id="regType" onchange="changeregType();"   datatype="*" nullmsg="请选择企业类型" errormsg="请选择企业类型">
								<c:forEach items="${regulatoryTypes}" var="rtype">
									<option value="${rtype.id}" <c:if test="${regulatoryType.id == rtype.id}">selected="selected"</c:if>>${rtype.regType}</option>
								</c:forEach>
						</select></li>
						<li>
						...
						<li class="cs-in-style cs-md-w"><input type="text" name="creditCode" <c:if test="${!empty regulatoryObject}">value="${regulatoryObject.creditCode}"</c:if> /></li>
						<li class="cs-name cs-sm-w">法人名称：</li>
						<li class="cs-in-style cs-md-w"><input type="text" name="legalPerson" <c:if test="${!empty regulatoryObject}">value="${regulatoryObject.legalPerson}"</c:if> /></li>
						<li class="cs-name cs-sm-w">联系人名称：</li>
						<li class="cs-in-style cs-md-w"><input type="text" name="linkUser" <c:if test="${!empty regulatoryObject}">value="${regulatoryObject.linkUser}"</c:if> /></li>

						<li class="cs-name cs-sm-w">联系人电话：</li>
						<li class="cs-in-style cs-md-w"><input type="text" name="linkPhone" <c:if test="${!empty regulatoryObject}">value="${regulatoryObject.linkPhone}"</c:if> /></li>

						<li class="cs-name cs-sm-w">联系人身份证：</li>
						<li class="cs-in-style cs-md-w"><input type="text" name="linkIdcard" <c:if test="${!empty regulatoryObject}">value="${regulatoryObject.linkIdcard}"</c:if> /></li>
						<li class="cs-name cs-sm-w">传真：</li>
						<li class="cs-in-style cs-md-w"><input type="text" name="fax" <c:if test="${!empty regulatoryObject}">value="${regulatoryObject.fax}"</c:if> /></li>
						<li class="cs-name cs-sm-w">排序：</li>
						<li class="cs-in-style cs-md-w"><input type="text" name="sorting" <c:if test="${!empty regulatoryObject}">value="${regulatoryObject.sorting}"</c:if> /></li>
				
						<li class="cs-name cs-sm-w">邮编：</li>
						<li class="cs-in-style cs-md-w"><input type="text" name="post" <c:if test="${!empty regulatoryObject}">value="${regulatoryObject.post}"</c:if> /></li>
						<li class="cs-name cs-sm-w">通讯地址：</li>
						<li class="cs-in-style cs-md-w"><input type="text" name="regAddress"  <c:if test="${!empty regulatoryObject}">value="${regulatoryObject.regAddress}"</c:if> /></li>
						<li class="cs-name cs-sm-w">经度纬度：</li>
						<li class="cs-in-style cs-md-w">
							<div class="cs-all-ps">
								<div class="cs-input-box">
									<input type="text" name="placeXY" <c:if test="${!empty regulatoryObject}">value="${regulatoryObject.placeX},${regulatoryObject.placeY}"</c:if> readonly="readonly"> <input type="hidden" name="placeX" <c:if test="${!empty regulatoryObject}">value="${regulatoryObject.placeX}"</c:if>> <input type="hidden" name="placeY" <c:if test="${!empty regulatoryObject}">value="${regulatoryObject.placeY}"</c:if>>
								</div>
							</div>
						</li>
						<li class="cs-name cs-sm-w audit cs-hide">状态：</li>
						<li class="cs-in-style cs-md-w audit cs-hide cs-radio"><input id="cs-check-radio0" type="radio" value="0" name="checked" <c:if test="${empty regulatoryObject || regulatoryObject.checked eq '0'}">checked="checked"</c:if> /><label for="cs-check-radio0">未审核</label> <input id="cs-check-radio1" type="radio" value="1" name="checked" <c:if test="${!empty regulatoryObject && regulatoryObject.checked eq '1'}">checked="checked"</c:if> /><label for="cs-check-radio1">已审核</label></li>
						</ul>
						<ul class="cs-add-new  cs-gh clearfix" style="height:80px;">
						<li class="cs-name cs-sm-w" style="line-height: 18px">经营范围：</li>
						<li class="cs-in-style cs-md-w"><textarea name="businessCope" maxlength="100" rows="" cols="" style="width: 200px; height: 50px;">${regulatoryObject.businessCope}</textarea></li>
						...
						<div class="cs-content">
							...
							<table class="cs-add-new">
								...
								<tr>
									...
									<td class="cs-in-style cs-radio"><input id="license-radio0" type="radio" value="0" name="checked" checked="checked" /><label for="license-radio0">未审核</label> <input id="license-radio1" type="radio" value="1" name="checked" /><label for="license-radio1">已审核</label></td>
								</tr>
							</table>
						</div>
					</form>
					...
					<form id="businessForm" enctype="multipart/form-data">
						...
						<div class="cs-content">
							...
							<table class="cs-add-new">
								...
								<tr>
									<td class="cs-name">状态：</td>
									<td class="cs-in-style cs-radio"><input id="business-radio0" type="radio" value="0" name="regulatoryBusiness.checked" checked="checked" /><label for="business-radio0">未审核</label> <input id="business-radio1" type="radio" value="1" name="regulatoryBusiness.checked" /><label for="business-radio1">已审核</label></td>
								</tr>
								<tr>
									<td>营业执照信息</td>
								</tr>
	...
	changeregType();
		$(function() {
			for (var i = 0; i < childBtnMenu.length; i++) {
				if (childBtnMenu[i].operationCode == "1396-4") {
					//审核权限
					$(".audit").show();
				}
			}
					...
					callback : function(data) {
						if (data && data.success) {
							...
							if (saveType == 1) {
								...
								self.location = '${webRoot}/ledger/regulatoryObject/ledgerAddRegulatoryObject.do?id='
										+ data.obj.id + '&regTypeId=' + $("select[name='regType']").val();
							}
