  ...
  <body>
          ...
          <!-- 面包屑导航栏  结束-->
            <div class="cs-search-box cs-fr">
              <form>
                <input class="cs-input-cont cs-fl focusInput" type="text" name="chainUnit.regName" placeholder="请输入企业名称" />
    ...
    for(var i = 0; i < childBtnMenu.length; i++) {
			if (childBtnMenu[i].operationCode == "1477-1") {
				//新增
				var html='<a class="cs-menu-btn" href="${webRoot}/cold/unit/goAddRegulatoryObject.do?regTypeCode=coldUnits&id=-200"><i class="'+childBtnMenu[i].functionIcon+'"></i>新增</a>';
				$("#showBtn").append(html);
			}else if (childBtnMenu[i].operationCode == "1477-2") {
				//编辑
				edit = 1;
				editObj=childBtnMenu[i];
			}else if (childBtnMenu[i].operationCode == "1477-3") {
				//删除
				deletes = 1;
				deleteObj=childBtnMenu[i];
			}else if (childBtnMenu[i].operationCode == "1477-5") {
				qrcodeBtn = 1;
				qrcodeObj=childBtnMenu[i];
			}else if (childBtnMenu[i].operationCode == "1477-8") {
				//监管对象导入
				objectExports = 1;
				objectExportObj=childBtnMenu[i];
			}else if (childBtnMenu[i].operationCode == "1477-9") {
				//经营户导入
				businessExports = 1;
    ...
    function bigbang(e) {
        ...
        if("${showBusiness}" == "false"){
        		...
        		op = {
        			...
        			tableAction: '${webRoot}'+"/cold/unit/datagrid.do?did="+did,	//加载数据地址
        			parameter: [		//列表拼接参数
        			    {
        			...
        			defaultCondition: [	//默认查询条件
        				{
        					queryCode: "chainUnit.regType",
        					queryVal: '${regTypeCode}'
        				}
        			],
        			funBtns: [
        				    	{
        				    		...
        				    		style: editObj,
        				    		action: function(id){
        				    			self.location = '${webRoot}/cold/unit/goAddRegulatoryObject.do?regTypeCode=coldUnits&id='+id;
        				    		}
        				    	},
        				    	{
        				    		show: deletes,
        				    		...
        				    		action: function(id){
        				    			if(id == ''){
        	    			    			$("#confirm-warnning .tips").text("请选择冷链单位");
        		    ...
        		    bottomBtns: [//底部按钮
        			    	...
        			    	{//导入监管对象按钮
        			    		...
        			    		action: function(ids){
        			    			location.href='${webRoot}/cold/unit/toImport.do'
        			    		}
        			    	},
        			    	{//导入经营户按钮
    		    ...
    		    bottomBtns: [
    			    	...
    			    	{//导入经营户按钮
    			    		show: businessExports,
    			         	style: businessExportObj,
    			    		action: function(ids){
    			    			location.href='${webRoot}/inspection/unit/toImport.do?from=cold'
    			    		}
    			    	}
    			    ],	//底部按钮
    		};
    ...
    $(document).on("click",".businessNumber",function(){
    	self.location = '${webRoot}/inspection/unit/list.do?regId='+$(this).parents(".rowTr").attr("data-rowId");
    });
    ...
    function deleteData(){
    	$.ajax({
	        type: "POST",
	        url: '${webRoot}'+"/cold/unit/delete.do",
    ...
    function viewQrcode(ids){
    	...
    	$.ajax({
	        type: "POST",
	        url: '${webRoot}'+"/cold/unit/regObjectQrcode.do",
