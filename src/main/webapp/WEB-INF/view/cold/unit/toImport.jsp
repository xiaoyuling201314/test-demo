...
<body>
		...
		<div class="wrapper">
			<form class="" id="saveForm" enctype="multipart/form-data">
				...
				<div class="is-flex">
					...
					<div class="">
						<p>
							<b><font color="#000000" size="4">冷链单位导入要求：</font></b>
						</p>
						<p>
							1、必须是EXCEL格式。 <a class="text-primary cs-link" href="${webRoot}/js/coldchainunit_importtemplate.xlsx">下载模版</a>
					...
					<%--<div class="col-sm-8">--%>
						<%--<p><b><font color="#000000" size="4">监管对象批量导入注意事项：</font></b></p>--%>
						<%--<p><font color="#000000">1、文件必须是EXCEL格式。--%>
						<%--<a href="${webRoot}/js/object_importtemplate.xlsx" style="text-decotation:underline;color:#FF0000;">点击此处下载模版</a></font>--%>
						<%--<br />--%>
					...
					<div  class="">
						...
						<c:choose>
							<c:when test="${isNewMenu=='N'}">
								<c:if test="${type==2}">
									<a class="btn btn-default" href="${webRoot}/dataCheck/recording/importList?importType=2">返回</a>
								</c:if>
								<c:if test="${type!=2}">
									<a class="btn btn-default" href="${webRoot}/cold/unit/list.do?regTypeCode=${regulatoryType.regTypeCode}">返回</a>
								</c:if>
							</c:when>
							<c:when test="${isNewMenu=='Y'}">
								<a class="btn btn-default" href="${webRoot}/ledger/regulatoryObject/list_new.do"><i class="icon iconfont icon-fanhui"></i>返回</a>
							</c:when>
							<c:otherwise>
								<a class="btn btn-default" href="javascript:;" onclick="self.history.back();"><i class="icon iconfont icon-fanhui"></i>返回</a>
							</c:otherwise>
						</c:choose>
					</div>
				</div>
	...
	<script type="text/javascript">
			...
			if($("#departId").val()!=""){
				if($("#kv-explorer").val()!=""){
					...
					$("#myModal-mid4").modal("show");
		  			...
		  			$.ajax({
		  		        type: "POST",
		  		        url: '${webRoot}'+"/cold/unit/importData",
		  		        data: formData,
		  		        ...
		  		        dataType: "json",
		  		      	success: function(data){
		  		      		...
		  		      		if(data.success==true){
			  		      		$("#myModalLabel").text("导入完成");
			  		      		$("#confirm-sure").modal('toggle');
		  		      		}else {
		  		      			if('${type}'){
			  		      			window.location.href="${webRoot}/dataCheck/recording/importList?importType=2";
		  		      			}else{
		  		      				if(data.msg){
		  		      					window.location.href="${webRoot}/resources/object/"+data.msg;
		  		      				}else {
	  ...
	  //上传控件
        ...
        console.log("isNewMenu = '${isNewMenu}'");
