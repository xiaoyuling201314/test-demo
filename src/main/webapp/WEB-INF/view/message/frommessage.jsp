...
<body>
	<div class="cs-col-lg clearfix">
          <!-- 面包屑导航栏  开始-->
            <ol class="cs-breadcrumb cs-fl">
              ...
              <li class="cs-b-active cs-fl">已发送
              </li>
            </ol>
          ...
          <!-- 面包屑导航栏  结束-->
           <div class="cs-search-box cs-fr">
              <form action="">
                ...
                <input class="cs-input-cont cs-fl focusInput" type="text" placeholder="请输入搜索内容..." name="tbTaskMessgae.title" />
                ...
                <a class="cs-menu-btn" href="${webRoot}/message/sendmessage" data-toggle="modal" name="content_frm"><i class="icon iconfont icon-zengjia"></i>发布</a>
              </div>
              </form>
  ...
  //遍历操作权限
    for (var i = 0; i < childBtnMenu.length; i++) {
		if (childBtnMenu[i].operationCode == "393-4") {
			view = 1;
			viewObj=childBtnMenu[i];
		}else if(childBtnMenu[i].operationCode == "393-5"){
			del = 1;
			delObj=childBtnMenu[i];
		}else if(childBtnMenu[i].operationCode == "393-6"){
			download = 1;
   	...
   	var op={
			...
			onload: function(){
		    	...
		    	//alert(datagridOption["departName"]);
			...
			parameter: [		//列表拼接参数
					...
					},
					{
						...
						query: 1,
						columnWidth:'30%',
						queryCode: "tbTaskMessgae.title",
						customElement: "<a class=\"bb\" href=\"javascript:;\">?</a>"
					},
					{
						columnCode: "filename",
						columnName: "附件",
				...
				{	//查看按钮
		    		...
		    		style: viewObj,
		    		action: function(id){
		    			window.location = '${webRoot}'+"/message/frommessagedetail.do?id="+id;
		    		}
		    	},{
		    		show: download,
		    		style: downloadObj,
  ...
  //查看详情
    $(document).on("click",".bb",function(){
    	...
    	self.location = '${webRoot}/message/frommessagedetail?id='+id;
    });
