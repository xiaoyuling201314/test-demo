...
<body>
  ...
  //遍历操作权限
    for (var i = 0; i < childBtnMenu.length; i++) {
		if (childBtnMenu[i].operationCode == "416-6") {
			view = 1;
			viewObj=childBtnMenu[i];
		}else if(childBtnMenu[i].operationCode == "416-7"){
			del = 1;
			delObj=childBtnMenu[i];
		}else if(childBtnMenu[i].operationCode == "416-8"){
			download = 1;
    ...
    var op={
			...
			parameter: [		//列表拼接参数
				...
				{	//查看按钮
		    		...
		    		style: viewObj,
		    		action: function(id){
		    			window.location = '${webRoot}'+"/message/tomessagedetail.do?id="+id;
		    		}
		    	},{
		    		show: download,
		    		style: downloadObj,
