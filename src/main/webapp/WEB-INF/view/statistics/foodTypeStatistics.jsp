            ...
            <span class="check-date cs-hide">
               <!-- <span class="cs-name"></span> -->
	                <span class="cs-in-style cs-time-se cs-time-se">
	                	<input name="task.taskSdate" style="width: 120px;" class="cs-time Validform_error" id="start" type="text" onclick="WdatePicker({maxDate:'#F{$dp.$D(end)}',startDate:'%y-%M-%d',maxDate:'%y-%M-%d',dateFmt:'yyyy-MM-dd',alwaysUseStartDate:true})" datatype="date" nullmsg="请选择任务起止时间" errormsg="请选择任务起止时间" value="" autocomplete="off">
	                	-
	                    <input name="task.taskSdate" style="width: 120px;" class="cs-time Validform_error" id="end" type="text" onclick="WdatePicker({minDate:'#F{$dp.$D(start)}',startDate:'%y-%M-%d',maxDate:'%y-%M-%d',dateFmt:'yyyy-MM-dd',alwaysUseStartDate:true})" datatype="date" nullmsg="请选择任务起止时间" errormsg="请选择任务起止时间" value="" autocomplete="off">
	                </span>
	                <span>
             ...
             <table class="cs-table cs-table-hover table-striped cs-tablesorter" id="table">
                <thead>
                  <tr>
                    ...
                    <th class="cs-header columnHeading" onclick="datagridUtil.orderBy2('table',1,'string')">食品种类<span class="sortIcon icon iconfont icon-sort"></span></th>
...
<div class="modal fade intro2" id="myModal-mid" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog cs-mid-width" role="document">
    ...
    for (var i = 0; i < childBtnMenu.length; i++) {
		if(childBtnMenu[i].operationCode == "1355-1"){
	...
	$("#month,#year1,#season,#year,#type,#pointType").change(function(){
		departConfirms();
    });
	function project() {
		 $.ajax({
	            	...
	            	$("#table tbody tr").remove();
	        			$.each(data.obj, function(k, v) {
	        				...
	        				var bigbang="<tr>"
	                            ...
	                            +"<td>"+v.foodName+"</td>"
	                            +"<td><a class='cs-link text-primary num' data-rowid="+v.id+">"+v.num+"</a></td>"
	                            +"<td><a class='cs-link text-primary qualified' data-rowid="+v.id+">"+qualifieds+"</a></td>"
	                            +"<td>"+test+"%</td>"
	                            +"<td><a class='cs-link text-primary unqualified' data-rowid="+v.id+">"+unqualifieds+"</a></td>"
	                            +"<td>"+text+"%</td>"
	                          	+"</tr>"
    ...
    $(document).on("click",".num",function(){
    	...
    	if (typeObj!=undefined) {
    		showMbIframe(encodeURI('${webRoot}/statistics/listStatistics?foodTypeId='+$(this).attr("data-rowId")+"&type="+type+"&month="+month+"&season="+season+"&year="+year+"&start="+start+"&end="+end+"&did="+did+"&dname="+dname+"&typeObj="+typeObj +"&pointType="+pointType));
		}else {
			showMbIframe(encodeURI('${webRoot}/statistics/listStatistics?foodTypeId='+$(this).attr("data-rowId")+"&type="+type+"&month="+month+"&season="+season+"&year="+year+"&start="+start+"&end="+end+"&did="+did+"&dname="+dname +"&pointType="+pointType));
		}
    });
    ...
    $(document).on("click",".qualified",function(){
        ...
        var pointType=$("select[name=pointType]").val();
    	if (typeObj!=undefined) {
    		showMbIframe(encodeURI('${webRoot}/statistics/listStatistics?foodTypeId='+$(this).attr("data-rowId")+"&type="+type+"&month="+month+"&season="+season+"&year="+year+"&start="+start+"&end="+end+"&did="+did+"&dname="+dname+"&conclusion=0"+"&typeObj="+typeObj +"&pointType="+pointType));
		}else {
			showMbIframe(encodeURI('${webRoot}/statistics/listStatistics?foodTypeId='+$(this).attr("data-rowId")+"&type="+type+"&month="+month+"&season="+season+"&year="+year+"&start="+start+"&end="+end+"&did="+did+"&dname="+dname+"&conclusion=0" +"&pointType="+pointType));
		}
    });
  ...
  //查看不合格数
    $(document).on("click",".unqualified",function(){
        ...
        var pointType=$("select[name=pointType]").val();
    	if (typeObj!=undefined) {
    		showMbIframe(encodeURI('${webRoot}/statistics/listStatistics?foodTypeId='+$(this).attr("data-rowId")+"&type="+type+"&month="+month+"&season="+season+"&year="+year+"&start="+start+"&end="+end+"&did="+did+"&dname="+dname+"&conclusion=1"+"&typeObj="+typeObj +"&pointType="+pointType));
		}else {
			showMbIframe(encodeURI('${webRoot}/statistics/listStatistics?foodTypeId='+$(this).attr("data-rowId")+"&type="+type+"&month="+month+"&season="+season+"&year="+year+"&start="+start+"&end="+end+"&did="+did+"&dname="+dname+"&conclusion=1" +"&pointType="+pointType));
		}
    });
