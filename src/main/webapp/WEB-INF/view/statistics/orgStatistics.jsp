...
<html>
	...
	<body>
          ...
          <!-- 面包屑导航栏  开始-->
            <ol class="cs-breadcrumb">
              ...
              <li class="cs-b-active cs-fl">被检单位统计
              </li>
            </ol>
          ...
          <div class="cs-input-style cs-fl" style="margin:3px 0 0 15px;" id="typeDiv">
              ...
              <%--被检单位任务类型筛选--%>
          <div class="cs-input-style cs-fl  taskType cs-hide" style="margin:3px 3px 0 15px;" id="taskTypeDiv">
              任务类型：
              <select id="taskType" class="cs-selcet-style" style="width: 85px;" onchange="loadData();" >
               ...
               <!-- <span class="cs-name">时间:</span> -->
                <input type="text">
	                <span class="cs-in-style cs-time-se cs-time-se">
	                	<input name="task.taskSdate" style="width: 120px;" autocomplete="off" class="cs-time Validform_error" id="start" type="text" onclick="WdatePicker()" datatype="date" nullmsg="请选择任务起止时间" errormsg="请选择任务起止时间" value="">
	                	<!-- <span style="padding:0 5px;">至</span> -->
	                    <input name="task.taskSdate" style="width: 120px;" autocomplete="off"  class="cs-time Validform_error" id="end" type="text" onclick="WdatePicker()" datatype="date" nullmsg="请选择任务起止时间" errormsg="请选择任务起止时间" value="">
	                </span>
	                <span>
             ...
             <table class="cs-table cs-table-hover table-striped cs-tablesorter" id="table">
                <thead>
                  <tr>
                    ...
                    <th class="cs-header columnHeading" onclick="datagridUtil.orderBy2('table',1,'string')">机构<span class="sortIcon icon iconfont icon-sort"></span></th>
                    <th class="cs-header columnHeading" onclick="datagridUtil.orderBy2('table',2,'string')">被检单位<span class="sortIcon icon iconfont icon-sort"></span></th>
                    <th class="cs-header columnHeading taskType cs-hide " onclick="datagridUtil.orderBy2('table',3,'int')">任务类型<span class="sortIcon icon iconfont icon-sort"></span></th>
                    <th class="cs-header columnHeading" onclick="datagridUtil.orderBy2('table',4,'int')">抽检总数<span class="sortIcon icon iconfont icon-sort"></span></th>
                    <th class="cs-header columnHeading" onclick="datagridUtil.orderBy2('table',5,'int')">合格数<span class="sortIcon icon iconfont icon-sort"></span></th>
                    <th class="cs-header columnHeading" onclick="datagridUtil.orderBy2('table',6,'float')">合格率<span class="sortIcon icon iconfont icon-sort"></span></th>
                    <th class="cs-header columnHeading" onclick="datagridUtil.orderBy2('table',7,'int')">不合格数<span class="sortIcon icon iconfont icon-sort"></span></th>
                    <th class="cs-header columnHeading" onclick="datagridUtil.orderBy2('table',8,'float')">不合格率<span class="sortIcon icon iconfont icon-sort"></span></th>
                  </tr>
                </thead>
                <tbody>
 ...
 <!-- Button to trigger modal -->
	...
	for (var i = 0; i < childBtnMenu.length; i++) {
		if(childBtnMenu[i].operationCode == "1311-1"){
			var html='<select id="type" class="cs-selcet-style" style="width: 85px;"></select>';
			$("#typeDiv").append(html);
		}
	}
    if(Permission.exist("1311-2")){
        $("#pointType").removeClass("cs-hide");
    }
    if(Permission.exist("1311-4")){
        $("#taskTypeDiv").removeClass("cs-hide");
        $(".taskType").removeClass("cs-hide");
    }
	var regId = [];
 	var regName = [];
 	var regNames = [];
	...
	function loadData() {
		$.ajax({
	        type: "POST",
	        url: "${webRoot}/statistics/loadReg.do",
	        	...
	        	if(data && data.success){
        			...
        			$.each(data.obj, function(k, v) {
        				...
        				var bigbang="<tr>"
                            ...
                            +"<td>"+v.departName+"</td>"
                            +"<td><a class='cs-link text-primary aa' data-rowid="+v.regId+">"+v.regName+"</a></td>";
                            if(Permission.exist("1311-4")){
                                bigbang+="<td>"+getTaskType(v.taskType)+"</td>";
                            }
                        bigbang+="<td><a class='cs-link text-primary num' data-rowid="+v.regId+">"+v.num+"</a></td>"
                            +"<td><a class='cs-link text-primary qualified' data-rowid="+v.regId+">"+v.qualified+"</a></td>"
                            +"<td>"+((v.qualified/v.num)*100).toFixed(2)+"%</td>"
                            +"<td><a class='cs-link text-primary unqualified' data-rowid="+v.regId+">"+v.unqualified+"</a></td>"
                            +"<td>"+((v.unqualified/v.num)*100).toFixed(2)+"%</td>"
                          	+"</tr>"
                          	$("#table tbody").append(bigbang);
        				});
        			    let colspanNum=Permission.exist("1311-4") ? 3 : 2;
	        			$("#table tbody").append("<tr><td style='width:50px;'>"+(data.obj.length+1)+"</td><td colspan='"+colspanNum+"'>合计</td><td>"+nums+"</td><td>"+qualifieds+"</td><td>"+(nums==0?0:((qualifieds/nums)*100).toFixed(2))+"%</td><td>"+unqualifieds+"</td><td>"+(nums==0?0:((unqualifieds/nums)*100).toFixed(2))+"%</td></tr>");

	        			arr.splice(echartMaxLength2);
	        			regId.splice(echartMaxLength1);
	        			regNames.splice(echartMaxLength2);
	        			regName.splice(echartMaxLength1);
    ...
    $(document).on("click",".aa",function(){
    	...
    	if(typeObj!=undefined){
        	<%--self.location = encodeURI('${webRoot}/statistics/orgStatisticsMon?regId='+$(this).attr("data-rowId")+"&type="+type+"&month="+month+"&season="+season+"&year="+year+"&start="+start+"&end="+end+"&did="+did+"&dname="+dname+"&typeObj="+typeObj);--%>
            urlPath=encodeURI('${webRoot}/statistics/orgStatisticsMon?regId='+$(this).attr("data-rowId")+"&type="+type+"&month="+month+"&season="+season+"&year="+year+"&start="+start+"&end="+end+"&did="+did+"&dname="+dname+"&typeObj="+typeObj+"&pointType="+pointType);
        }else {
        	<%--self.location = encodeURI('${webRoot}/statistics/orgStatisticsMon?regId='+$(this).attr("data-rowId")+"&type="+type+"&month="+month+"&season="+season+"&year="+year+"&start="+start+"&end="+end+"&did="+did+"&dname="+dname);--%>
            urlPath= encodeURI('${webRoot}/statistics/orgStatisticsMon?regId='+$(this).attr("data-rowId")+"&type="+type+"&month="+month+"&season="+season+"&year="+year+"&start="+start+"&end="+end+"&did="+did+"&dname="+dname+"&pointType="+pointType);
        }
    ...
    $(document).on("click",".num",function(){
        ...
        var pointType=$("select[name=pointType]").val();
    	if (typeObj!=undefined) {
    		showMbIframe(encodeURI('${webRoot}/statistics/listStatistics?regId='+$(this).attr("data-rowId")+"&type="+type+"&month="+month+"&season="+season+"&year="+year+"&start="+start+"&end="+end+"&did="+did+"&dname="+dname+"&typeObj="+typeObj+"&pointType="+pointType));
		}else {
			showMbIframe(encodeURI('${webRoot}/statistics/listStatistics?regId='+$(this).attr("data-rowId")+"&type="+type+"&month="+month+"&season="+season+"&year="+year+"&start="+start+"&end="+end+"&did="+did+"&dname="+dname+"&pointType="+pointType));
		}
    });
  	//查看合格数
    $(document).on("click",".qualified",function(){
        ...
        var pointType=$("select[name=pointType]").val();
    	if (typeObj!=undefined) {
    		showMbIframe(encodeURI('${webRoot}/statistics/listStatistics?regId='+$(this).attr("data-rowId")+"&type="+type+"&month="+month+"&season="+season+"&year="+year+"&start="+start+"&end="+end+"&did="+did+"&dname="+dname+"&conclusion=0"+"&typeObj="+typeObj+"&pointType="+pointType));
		}else {
			showMbIframe(encodeURI('${webRoot}/statistics/listStatistics?regId='+$(this).attr("data-rowId")+"&type="+type+"&month="+month+"&season="+season+"&year="+year+"&start="+start+"&end="+end+"&did="+did+"&dname="+dname+"&conclusion=0"+"&pointType="+pointType));
		}
    });
  ...
  //查看不合格数
    $(document).on("click",".unqualified",function(){
    	...
    	var typeObj=$("#type option:selected").val();
    	if (typeObj!=undefined) {
    		showMbIframe(encodeURI('${webRoot}/statistics/listStatistics?regId='+$(this).attr("data-rowId")+"&type="+type+"&month="+month+"&season="+season+"&year="+year+"&start="+start+"&end="+end+"&did="+did+"&dname="+dname+"&conclusion=1"+"&typeObj="+typeObj));
		}else {
			showMbIframe(encodeURI('${webRoot}/statistics/listStatistics?regId='+$(this).attr("data-rowId")+"&type="+type+"&month="+month+"&season="+season+"&year="+year+"&start="+start+"&end="+end+"&did="+did+"&dname="+dname+"&conclusion=1"));
		}
    });
