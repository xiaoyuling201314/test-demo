...
<html>
  <head>
            ...
            <span class="check-date pull-left cs-hide">
              <!--  <span class="cs-name">时间:</span> -->
	                <span class="cs-in-style cs-time-se cs-time-se">
	                	<input name="task.taskSdate" id="start" style="width: 120px;" autocomplete="off" class="cs-time Validform_error" type="text" onclick="WdatePicker()" datatype="date" nullmsg="请选择任务起止时间" errormsg="请选择任务起止时间" value="">
	                	<!-- <span style="padding:0 5px;">至</span> -->
	                    <input name="task.taskSdate" id="end" style="width: 120px;" autocomplete="off" class="cs-time Validform_error" type="text" onclick="WdatePicker()" datatype="date" nullmsg="请选择任务起止时间" errormsg="请选择任务起止时间" value="">
	                </span>
	                <span>
             ...
             <table class="cs-table cs-table-hover table-striped cs-tablesorter" id="table">
                <thead>
                  <tr>
                    ...
                    <th class="cs-header columnHeading" onclick="datagridUtil.orderBy2('table',2,'string')">检测点<span class="sortIcon icon iconfont icon-sort"></span></th>
                    <th class="cs-header columnHeading" onclick="datagridUtil.orderBy2('table',3,'int')">抽检总数<span class="sortIcon icon iconfont icon-sort"></span></th>
                    <th class="cs-header columnHeading" onclick="datagridUtil.orderBy2('table',4,'int')">合格数<span class="sortIcon icon iconfont icon-sort"></span></th>
                    ...
                    <th class="cs-header columnHeading" onclick="datagridUtil.orderBy2('table',6,'int')">不合格数<span class="sortIcon icon iconfont icon-sort"></span></th>
                    <th class="cs-header columnHeading" onclick="datagridUtil.orderBy2('table',7,'float')">不合格率<span class="sortIcon icon iconfont icon-sort"></span></th>
                </tr>
                </thead>
                <tbody>
                </tbody>
    ...
    for (var i = 0; i < childBtnMenu.length; i++) {
		if(childBtnMenu[i].operationCode == "1308-1"){
		        	...
		        	if(data && data.success){
	        			...
	        			$.each(data.obj, function(k, v) {
	        				...
	        				var bigbang="<tr>"
	                            ...
	                            +"<td>"+v.departName+"</td>"
	                            +"<td><a class='cs-link text-primary aa' data-rowid="+v.pointId+">"+v.pointName+"</a></td>"
	                            +"<td><a class='cs-link text-primary num' data-rowid="+v.pointId+">"+v.num+"</a></td>"
	                            +"<td><a class='cs-link text-primary qualified' data-rowid="+v.pointId+">"+v.qualified+"</a></td>"
	                            +"<td>"+((v.qualified/v.num)*100).toFixed(2)+"%</td>"
	                            +"<td><a class='cs-link text-primary unqualified' data-rowid="+v.pointId+">"+v.unqualified+"</a></td>"
	                            +"<td>"+((v.unqualified/v.num)*100).toFixed(2)+"%</td>"
	                          	+"</tr>"
	                          	$("#table tbody").append(bigbang);
	        				});
		        			$("#table tbody").append("<tr><td style='width:50px;'>"+(data.obj.length+1)+"</td><td colspan='2'>合计</td><td>"+nums+"</td><td>"+qualifieds+"</td><td>"+(nums==0?0:((qualifieds/nums)*100).toFixed(2))+"%</td><td>"+unqualifieds+"</td><td>"+(nums==0?0:((unqualifieds/nums)*100).toFixed(2))+"%</td></tr>");
