        ...
        <div class="modal-content" style="margin-top: 0px;">
            ...
            <div class="modal-body cs-xlg-height">
                ...
                <div class="cs-mechanism-list">
                    <div class="cs-mechanism-list-title cs-font-weight">所属机构列表</div>
                ...
                <div class="cs-points-list cs-points-list2">
                    ...
                    <div class="cs-mechanism-list-content cs-mechanism-list-content2" style="padding:0;">
                        <div class="cs-mechanism-list-search clearfix">
                            ...
                            <div class="cs-fr">
                                <div class="cs-search-margin clearfix cs-fl">
                                    <input class="cs-input-cont cs-fl" type="text" placeholder="请输入人员名称" name="searchPointName">
                        ...
                        <div class="cs-mechanism-table-box cs-mechanism-table-box2">
                            <table class="cs-mechanism-table" width="100%" id="pointsList" style="table-layout:;fixed">
                                <tr>
                ...
                <div class="cs-secleted-list">
                    <div class="cs-mechanism-list-title">
                        <span class="cs-fl cs-font-weight">已选择人员：</span>
...
<!-- 修改人员信息 -->
<div class="modal fade intro2" id="myEditUserModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog cs-sm-width" role="document">
        <div class="modal-content ">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="myModalLabel">修改人员信息</h4></div>
                        ...
                        <form class="registerform" method="post">
                            ...
                            <div width="100%" class="cs-add-new">
                                <ul class="cs-ul-form clearfix">
                                    <li class="cs-name col-md-3">名称：</li>
                                    <li class="cs-in-style col-md-5">
                                        <span class="workerName"></span>
                                    </li>
                                </ul>
                                <ul class="cs-ul-form clearfix">
                                    <li class="cs-name col-md-3">职位：</li>
                                    <li class="cs-in-style col-md-5">
                                ...
                                <ul class="cs-ul-form clearfix">
                                    <li class="cs-name col-md-3">工作内容：</li>
                                    <li class="cs-in-style col-md-5">
            ...
            <div class="modal-footer action">
                <button type="submit" class="btn btn-success" onclick="updatePointUser();">确定</button>
...
<script type="text/javascript">
    ...
    function loadPoints() {
        ...
        $.ajax({
            type: "POST",
            url: '${webRoot}' + "/data/workers/queryWorkers.do",
            data: {"departId": checkedDepartId, "userName": pointName},
            dataType: "json",
                        ...
                        if (pointsArray.length > 0) {
                            for (var ii = 0; ii < pointsArray.length; ii++) {
                                if (data.obj[i].id == pointsArray[ii]["id"]) {
                                    //已选
                                    var html = "<tr><td><input type=\"checkbox\" checked=\"checked\" class=\"pointItem\" value=\"" + data.obj[i].id + "\" data-pointName=\"" + data.obj[i].workerName + "\"/></td>"
                                        + "<td>" + data.obj[i].workerName + "</td><td>" + data.obj[i].pointsNameStr + "</td></tr>";
                                    $("#pointsList").append(html);
                                    break;
                                } else if (ii == pointsArray.length - 1) {
                                    //未选
                                    var html = "<tr><td><input type=\"checkbox\" class=\"pointItem\" value=\"" + data.obj[i].id + "\" data-pointName=\"" + data.obj[i].workerName + "\"/></td>"
                                        + "<td>" + data.obj[i].workerName + "</td><td>" + data.obj[i].pointsNameStr + "</td></tr>";
                                    $("#pointsList").append(html);
                                }
                            }
    ...
    $(function () {
        ...
        $.ajax({
            type: "POST",
            url: "${webRoot}/system/tstype/queryByTypeCode.do",
            data: {"typeCode": "position"},
            dataType: "json",
            success: function (data) {
                if (data && data.success && data.obj) {
                    var tsTypes = data.obj.tsTypes;
                    for (var i = 0; i < tsTypes.length; i++) {
                        $(".registerform select[name='positionCode']").append("<option value=\"" + tsTypes[i].typeValue + "\">" + tsTypes[i].typeName + "</option>");
                    }
                }
            }
        });
    ...
    function updatePointUser() {
        $.ajax({
            type: "POST",
            url: "${webRoot}/data/pointUser/update.do",
            ...
            success: function (data) {
                if (data && data.success) {
                    ...
                    $("#myEditUserModal").modal('toggle');
                    $("#myTipsModal .tips").text('删除失败');
                    $("#myTipsModal").modal('toggle');
                }
            }
        });
    }
