...
<!-- 选择检测标准 - 下拉框 -->
<select id="standard_select2" class="js-data-example-ajax"></select>
<script type="text/javascript" src="${webRoot}/js/jquery.cookie.js"></script>
<script type="text/javascript">
	$("#standard_select2").select2({
		placeholder: '--请选择--',
		language: {
		...
		width: "100%",
		ajax: {
			url: "${webRoot}/data/standard/select2StandardData",
			dataType: 'json',
			delay: 250,
			data: function(params) {//发送到服务器的数据
				...
				return {
                    standardCode: (!params.term ? '本地历史数据' : params.term), //后端取 key 它就是搜索关键字
			...
			processResults: function(data, params) {// 后端返数据的根是 data，params就是上面的查寻参数
                //查询历史数据
                if (!params.term) {
                    if ($.cookie("STANDARD_SEL")) {
                        return {
                            results: JSON.parse($.cookie("STANDARD_SEL")),
                            pagination: { //分页
     ...
     * 获取已选数据数组
     */
	function getStandardSelect2Data() {
        var selData = $('#standard_select2').select2('data');
        // //录入数据不保存到历史
        // if (selData[0].id > 0) {
            var STANDARD_SEL;
            if (!$.cookie("STANDARD_SEL")) {
                STANDARD_SEL = [];
                STANDARD_SEL.push({"id": selData[0].id, "name": selData[0].name, "number": 1});
            } else {
                STANDARD_SEL = JSON.parse($.cookie("STANDARD_SEL"));
                for (var i=0; i<STANDARD_SEL.length; i++) {
                    if (STANDARD_SEL[i].id == selData[0].id) {
                        STANDARD_SEL[i].number = STANDARD_SEL[i].number + 1;
                        break;
                    } else if (i == STANDARD_SEL.length-1) {
                        STANDARD_SEL.push({"id": selData[0].id, "name": selData[0].name, "number": 1});
                        break;
                    }
                }
            }
            STANDARD_SEL.sort(function (a, b) {
            ...
            $.cookie("STANDARD_SEL", JSON.stringify(STANDARD_SEL));
        // }

        return $('#standard_select2').select2('data');
    }

    /**
     * 清除选择项
     */
    function cleanStandardSelected() {
        $("#select2-standard_select2-container").text("--请选择--");
        $("#standard_select2").select2('val', "");
        $("#standard_select2").select2('data', null);
        $("#standard_select2").empty();
    }
</script>
