...
<body class="easyui-layout">
	...
	<div data-options="region:'north',border:false" style="top: 0px; border: none;">
		...
		<div class="cs-search-box cs-fr">
        	<div class="clearfix" id="showBtn"></div>    
        </div>
		</div>
	</div>
					...
					<div class="modal-body cs-mid-height">
						...
						<div class="cs-tabcontent">
							<div class="cs-content2">
								...
								<input type="hidden" name="pointName">
									 <tr>
										...
										<td class="cs-in-style">
											<div class="cs-all-ps">
												<div class="cs-input-box">
													...
													<input type="hidden" name="departId" datatype="*" ignore="ignore" nullmsg="请选择所属机构" errormsg="请选择所属机构">
													<div class="cs-down-arrow"></div>
												</div>
												<div class="cs-check-down cs-hide" style="display: none;">
									...
									<tr>
										<td class="cs-name"><font color="#FF0000">*</font>所属检测室：</td>
										<td class="cs-in-style">
											<input type="hidden" name="pointName">
...
<!-- 												<option value="2">内网</option> -->
				...
				parameter : [ //列表拼接参数
					{
						...
						columnName : "监控类型",
						columnWidth : "10%",
						customVal: {"0":"乐橙API","1":"乐橙云","3":"萤石云API","4":"萤石云"}
					},  {
						columnCode : "param1",
						columnName : "通道数量",
						columnWidth : "80px",
		...
		function dealHtml(data){
			...
			if(data=="") {
				...
				if(htmlStr!=""){
					...
					$("#pointSelect").append('<option value="'+json[0].id+'">'+json[0].pointName+'</option>');
					$("#pointSelect").val(json[0].id);
	...
	//     changeUserPhone();
			//新增标签
		if(Permission.exist("1475-1")){
			$("#showBtn").append('<a class="cs-menu-btn" href="javascript:;" onclick="editVideo(0)"><i class="'+Permission.getPermission("1475-1").functionIcon+'"></i>新增</a>');
		}
	                    ...
	                    for (var i = 0; i < data.obj.length; i++) {
							if(data.obj[i].accountPhone!="" && data.obj[i].videoType==0){
								...
								for(let j = 0; j < data.obj[i].param1; j++){
									...
									kitTokenStr+=accoumtMap["account_"+data.obj[i].accountPhone].kitToken+"%";
								}
								if(!flag1){
...
//                     $('#showBtn').css('display','none')
        ...
        function changeVideoType() {
			...
			} else if($("#camera").val() == '1' || $("#camera").val() == '4'){//H5直播方式
				$('.cs-lecheng').hide();
				$('.cs-yingshi').hide();
				$('.cs-neiwang').show();
		...
		var saveForm = $("#saveForm").Validform({
			beforeSubmit: function(data){
				...
				$("#saveForm input[name=pointName]").val($("#pointSelect option:checked").text());
...
//                             $("#surveillanceNameChose").val([data.obj.surveillanceName]).trigger('change');
                            ...
                            $("#saveForm input[name=accountPhone]").val(data.obj.accountPhone);
