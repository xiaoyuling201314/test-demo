 ...
 */
@Controller
@RequestMapping("/interfaces/devices")
public class DevicesController extends BaseInterfaceController {
    ...
    public AjaxJson registerDevice(HttpServletRequest request, String userToken, String series, String mac) {
        ...
        try {
            ...
            subffer.append("select d.* from base_device d "+
                    " INNER JOIN base_device_type t on d.device_type_id=t.id "+
                    " where d.delete_flag = 0 and d.mac_address =? AND d.point_id=? "+
                    " and REPLACE(t.device_series,'-','')=?");
            //根据mac地址查询仪器信息
            ...
            if (map.size() == 0) {//该仪器暂未注册过
                //2.根据用户ID和仪器系列查询可用仪器信息
                subffer.delete(0, subffer.length());
                subffer.append("select d.* from base_device d " +
                        " inner join base_device_type t on d.device_type_id=t.id"+
                        " where d.point_id =? and d.delete_flag=0 AND d.status = 0 " +
                        " and REPLACE(t.device_series,'-','')=? and (d.serial_number is NULL or d.serial_number ='')"+
                ...
                if (deviceType == null) {
                    throw new MyException("平台没有该系列仪器", "平台没有该"+series+"系列仪器,请先在平台【仪器类别】中添加仪器系列", WebConstant.INTERFACE_CODE12);
                } else if (map.size() > 0) {//有空闲仪器
                    //更新仪器信息，保存唯一编号和mac地址
                    Map<String, Object> mapBean = map.get(0);
                    bean = baseDeviceService.getById(mapBean.get("id").toString());
...
//                        bean = new BaseDevice(deviceCode, deviceType.getDeviceName(), "仪器设备", deviceType.getId(), user.getDepartId(), user.getPointId(), (short) 0, mac, serialNumber,date,date);
	...
	public AjaxJson registerDevice(HttpServletRequest request, String userToken, String series,String mac){
		...
		try {
			...
			StringBuffer subffer=new StringBuffer();
			subffer.append("  select d.* from base_device d ");
			subffer.append(" INNER JOIN base_device_type t on d.device_type_id=t.id ");
			subffer.append(" where d.delete_flag = 0 and d.mac_address =? AND d.point_id=? ");
			subffer.append(" and t.device_series=?");
			//根据mac地址查询仪器信息
			...
			if(map.size()==0){//该仪器暂未注册过
				...
				subffer.append("select d.* from base_device d");
				subffer.append(" inner join base_device_type t on d.device_type_id=t.id");
				subffer.append(" where d.point_id =(select point_id from t_s_user where id=?)");
				subffer.append(" and d.delete_flag=0 AND d.status = 0 and t.device_series=? and (d.serial_number is NULL or d.serial_number ='')");
				subffer.append(" order by d.update_date LIMIT 0,1");
				map=jdbcTemplate.queryForList(subffer.toString(), user.getId(),series);
				if(map.size()>0){
					...
					BaseDevice bean=baseDeviceMapper.selectByPrimaryKey(mapBean.get("id").toString());
					bean.setMacAddress(mac);
					bean.setSerialNumber(serialNumber);
					bean.setRegisterDate(new Date());
					baseDeviceMapper.updateByPrimaryKeySelective(bean);
					//3.设置返回仪器信息
					aj.setObj(mapBean);
				}else{//该检测点下没有该系列仪器
					throw new MyException("检测点下没有该系列仪器或均已注册", "检测点下没有该系列仪器或均已注册", WebConstant.INTERFACE_CODE13);
				}
			}else{//该仪器曾注册过，直接返回
				aj.setObj(map.get(0));
				if ("1".equals(map.get(0).get("status"))) {
